...
void make_ekran_setpoint_zz(unsigned int group)
{
  ...
  for (unsigned int i=0; i< MAX_ROW_LCD; i++)
  {
    ...
    if (
        ...
        &&  
        (index_of_ekran < (MAX_ROW_FOR_SETPOINT_ZZ<<1))/*Множення на два константи MAX_ROW_FOR_SETPOINT_ZZ потрібне для того, бо на одну позицію ми використовуємо два рядки (назва + значення)*/
       )   
    {
      if ((i & 0x1) == 0)
      {
